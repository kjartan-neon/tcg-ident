...
def identify_card_info(img_for_ocr, det_predictor, rec_predictor, card_database=None):
    """
    Runs Surya OCR and heuristic extraction on a given image.
    """
    ...
    # Run Surya text recognition with detection predictor
    # task_names should be one of: 'ocr_with_boxes', 'ocr_without_boxes', 'block_without_boxes', 'layout'
    task_names = ["ocr_without_boxes"]
    rec_predictions = rec_predictor([pil_image], task_names=task_names, det_predictor=det_predictor)
    
    # Extract text from Surya results
    detected_texts = []
    if rec_predictions and len(rec_predictions) > 0:
        prediction = rec_predictions[0]
        # Surya returns a list of TextLine objects with 'text' attribute
        for text_line in prediction.text_lines:
            detected_texts.append(text_line.text)
...
if __name__ == "__main__":
    ...
    print("Initializing Surya OCR (This may take a moment and download models the first time)...")
    try:
        ...
        print("Please check your Surya installation and its dependencies.")
    ...
    for path in image_paths:
        ...
        info = identify_card_info(img_to_process, det_predictor, rec_predictor, card_database)
